...
const fetchWithAuth = async (endpoint: string, options: RequestInit = {}) => {
  ...
  if (!response.ok) {
    // Try to parse JSON error, otherwise include raw text for debugging
    let errBody: any;
    try {
      errBody = await response.json();
    } catch (e) {
      errBody = { error: await response.text().catch(() => 'Unknown error') };
    }
    throw new Error(errBody.error || `HTTP ${response.status}`);
  }

  // Parse JSON, but if parsing fails include raw text for easier debugging
  try {
